...
import { deleteSkill, getSkill } from "@/store/masters/skill/skill";
...
/* import { getSkill, deleteFacility } from "@/store/Skill"; */ // future API helpers
...
export default function SkillPage() {
  const [skill, setSkill] = useState<FacilityType[]>([]);
  ...
  const fetchSkill = async () => {
    ...
    const formattedData = data.map((f: skillAllDataInterface) => ({
      ...f,
  ...
  const handleEdit = (id?: string) => {
    router.push(`/masters/skill/edit/${id}`);
  };
  ...
  const totalTablePages = Math.ceil(filteredSkill.length / rowsPerTablePage);
  const indexOfLastRow = currentTablePage * rowsPerTablePage;
  const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
  const currentRows = filteredSkill.slice(indexOfFirstRow, indexOfLastRow);
    ...
    <MasterProtectedRoute>
      ...
      <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
          <PageHeader title="Dashboard" subtitles={["Skill"]} />
          {/* Add Button */}

          <AddButton
            url="/masters/skill/add"
            text="Add"
