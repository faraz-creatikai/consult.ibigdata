...
function Tablesetting({ columns, setColumns }:TableSettingProps) {
  ...
  return (
    <>
       ...
       <div className="absolute top-[62px] md:right-0 xl:right-4   border-[var(--color-secondary-dark)] flex items-end justify-end z-40">
                    <button
                    onClick={()=>setTogglesetting(prev => !prev)}
                    className="bg-[var(--color-primary)]  z-40 w-14 h-16 text-white cursor-pointer flex justify-center items-center"><IoMdSettings className="hover:scale-105 " size={24}/></button>
                    </div>
                    ...
                    className={`absolute top-28 max-h-54 bg-white shadow-2xl shadow-gray-300
                        text-gray-900 px-1 py-2 right-18  flex-col justify-center items-start rounded-md   z-50 
